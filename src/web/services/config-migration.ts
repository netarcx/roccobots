...
/**
 * Import configuration from raw .env content string
 */
export async function importFromEnvContent(
  configService: ConfigService,
  content: string,
): Promise<{ created: number; errors: string[] }> {
  const env = parseEnvFile(content);
  return importFromEnvParsed(configService, env);
}

/**
 * Import configuration from .env file on disk
 */
export async function importFromEnv(
  ...
  try {
    ...
    return importFromEnvContent(configService, content);
  } catch (error) {
    return {
      created: 0,
      errors: [`Failed to read .env file: ${(error as Error).message}`],
    };
  }
}

/**
 * Import bots and platforms from parsed env variables
 */
async function importFromEnvParsed(
  configService: ConfigService,
  ...
  for (let i = 0; i < handles.length; i++) {
    ...
    try {
      ...
      const username =
        env[`TWITTER_USERNAME${postfix}`] || env.TWITTER_USERNAME;
      const password =
        env[`TWITTER_PASSWORD${postfix}`] || env.TWITTER_PASSWORD;
