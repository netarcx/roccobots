...
export async function uploadBlueskyVideo(
  ...
  // 2. Quota check (best-effort — if the endpoint fails, continue and let the
  //    upload itself surface the error).
  try {
    ...
    // Re-throw the "quota exhausted" signal; swallow transient fetch errors.
    if (err instanceof Error && err.message.startsWith("Video upload quota")) {
      throw err;
    }
  }
  ...
  // 4. Poll for processing completion with exponential backoff.
